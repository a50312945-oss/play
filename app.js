...
['bld_size','shade_pos','obs_pos',
 ...
 'ladder_exist','ladder_new','loc_pv_meter','loc_room','loc_hv','loc_taipower','loc_pole','loc_switch','route'
  ...
  mountPhotoPicker('ph-'+k, k, '插入照片');
});

// Radios -> show photo holders
...
document.getElementById('btnSaveJson').onclick = ()=>{
  ...
  a.download = '現勘勾選表-Mobile-v3.json'; a.click(); URL.revokeObjectURL(a.href);
};
...
document.getElementById('btnExport').onclick = async ()=>{
  try{
    ...
    const fname = `現勘報告_Mobile_v3_${new Date().toISOString().slice(0,16).replace(/[:T]/g,'-')}.pptx`;
